    ...
    projectsLoaded: {
      handler: function() {
        if (this.projectsLoaded) {
          this.ourProjects.data.items.filter(obj => {
            if (
              obj.fields.title.replace(/\s+/g, "-").toLowerCase() ===
              this.$route.params.name
            )
              this.project = obj.fields;
          });
...
<style scoped lang="scss" src="./Project.style.scss"></style>
