  ...
  <div class="port-container" v-if="projectsLoaded">
      ...
      <div
        ...
        @click="
          $router.push({
            name: 'Project',
            params: {
              name: item.fields.title.replace(/\s+/g, '-').toLowerCase()
            }
          })
        "
      >
    ...
    <div class="pages" v-if="projectsLoaded">
      ...
      <p
        ...
        :key="index"
        @click="currPage = n"
        :style="currPage === n ? { color: '#ef7a07' } : { color: '#999' }"
      >
        {{ n }}
      </p>
    ...
    filter(params, i) {
      ...
      if (params === "all") {
        ...
        this.pages = Math.ceil(
          this.ourProjects.data.items.length / this.itemsOnPage
        );
        this.currItems = this.ourProjects.data.items.slice(
          (this.currPage - 1) * 20,
          this.itemsOnPage
        );
      } else {
        ...
        this.currItems = this.filteredItems.slice(
          (this.currPage - 1) * 20,
          this.itemsOnPage
        );
      }
  ...
  watch: {
    ...
    currPage: function(val) {
      if (this.filteredItems.length === 0)
        this.currItems = this.ourProjects.data.items.slice(
          (val - 1) * 20,
          this.currPage < this.ourProjects.data.items.length / this.itemsOnPage
            ? this.currPage * this.itemsOnPage
            : this.itemsOnPage *
                (this.ourProjects.data.items.length / this.itemsOnPage)
        );
      if (this.filteredItems.length !== 0)
...
<style scoped lang="scss" src="./Portfolio.style.scss"></style>
